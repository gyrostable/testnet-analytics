...
def merge():
    ...
    with open("/Users/jonas/Workspace/Local/Drop/results/challenge-progression-stats-detail.csv") as a:
        late_levels = pd.read_csv(a,usecols=list(range(1,25)))
...
def condense(df:pd.DataFrame):
    ...
    overview['eligibility'] = pd.DataFrame(df[['lvl1','lvl2','lvl25','lvl3']].sum(axis=1))
    ...
    overview_filtered = overview[(overview['eligibility']>=3) & (overview['sybil_score']>=1)]
